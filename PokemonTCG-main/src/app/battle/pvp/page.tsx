...
export default function ArenaPage() {
  ...
  // Get available rarities from user's collection
  const getAvailableRarities = () => {
    if (!address || cards.length === 0) {
      return []; // No available rarities if wallet not connected or no cards
    }

    // Get unique rarities from user's cards
    const userRarities = Array.from(
      new Set(cards.map((c) => c.rarity).filter(Boolean))
    );

    // Normalize rarity names and match with CARD_RARITIES
    ...
    // Map user rarities to our rarity definitions
    const availableRarities = CARD_RARITIES.filter((rarity) => {
      return userRarities.some((userRarity) => {
        const normalized = normalizeRarity(userRarity);
        return (
          normalized === rarity.id ||
          userRarity.toLowerCase() === rarity.name.toLowerCase()
        );
      });
    });
  ...
  const getCardCountByRarity = (rarityId: string) => {
    if (!address || cards.length === 0) return 0;
    ...
    const matchingCards = cards.filter((card) => {
      ...
      return (
        ...
        card.rarity.toLowerCase() ===
          CARD_RARITIES.find((r) => r.id === rarityId)?.name.toLowerCase()
      );
    });

    return matchingCards.reduce(
      (total, card) => total + parseInt(card.amount),
      0
    );
  };
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      try {
        const res = await fetch("/api/battle/room/list", {
          method: "GET",
        });
        if (res.ok) {
  ...
  // Auto-select first available rarity when collection loads
  useEffect(() => {
    const availableRarities = getAvailableRarities();
    if (
      availableRarities.length > 0 &&
      !availableRarities.find((r) => r.id === selectedRarity)
    ) {
      setSelectedRarity(availableRarities[0].id);
    }
  }, [cards, address]);
  ...
  const handleCreateRoom = async () => {
    ...
    // Check if user has cards of the selected rarity
    const cardCount = getCardCountByRarity(selectedRarity);
    if (cardCount === 0) {
      const rarityInfo = getRarityInfo(selectedRarity);
      alert(`You don't have any ${rarityInfo.name} cards to create this room.`);
      return;
    }
  ...
  const handleJoinRoom = (room: Room) => {
    ...
    if (room.wagerRarity && !address) {
      alert("Please connect your wallet to join rooms with card wagering.");
      return;
    }

    // Check if user has cards for the room's rarity
    if (room.wagerRarity && address) {
      const cardCount = getCardCountByRarity(room.wagerRarity);
      if (cardCount === 0) {
        const rarityInfo = getRarityInfo(room.wagerRarity);
        alert(`You don't have any ${rarityInfo.name} cards to join this room.`);
        return;
      }
    }
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/battle/password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  // Get available rarities for current user
  const availableRarities = getAvailableRarities();
          ...
          {showCreateRoom && (
            ...
            >
              <div className="bg-black/30 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-2xl">
                ...
                {/* MODIFIED: Card Rarity Selection - Only Show Available Rarities */}
                <div className="mt-6">
                  <label className="block text-sm font-semibold mb-3 text-white/80">
                    🎴 Card Rarity {address && "(Your Available Cards)"}
                  </label>

                  {!address ? (
                    <div className="text-center p-8 bg-white/5 rounded-xl border border-white/10">
                      <p className="text-white/60 mb-4">
                        Connect your wallet to see available rarities
                      </p>
                      <button
                        onClick={connectWallet}
                        className="px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-400/30 rounded-lg text-blue-300 font-medium transition-colors"
                      >
                        🔗 Connect Wallet
                      </button>
                    </div>
                    ...
                    <div className="text-center p-8 bg-red-500/10 rounded-xl border border-red-400/30">
                      <p className="text-red-300 mb-4">
                        No cards found in your collection
                      </p>
                      <p className="text-red-200/80 text-sm">
                        You need to own Pokemon cards to create battle rooms
                      </p>
                    </div>
                  ) : (
                    <>
                      {/* Available Rarities Grid */}
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 max-h-96 overflow-y-auto custom-scrollbar">
                        {availableRarities.map((rarity) => {
                          const cardCount = getCardCountByRarity(rarity.id);
                          ...
                          return (
                            <motion.div
                              ...
                              className={`p-3 rounded-xl border-2 cursor-pointer transition-all duration-300 ${
                                isSelected
                                  ? `bg-gradient-to-br ${rarity.color} border-white/50 shadow-lg ring-2 ring-white/30`
                      ...
                      <div className="mt-4 p-3 bg-black/30 rounded-lg border border-white/10">
                        <div className="text-xs text-white/60 text-center">
                          ...
                          <span className="text-green-300">
                            {availableRarities.length} rarities
                          </span>
                        </div>
                      </div>
                    </>
          ...
          {rooms.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {rooms.map((room, index) => {
                const rarityInfo = room.wagerRarity
                  ? getRarityInfo(room.wagerRarity)
                  : null;
                const userHasRequiredCards =
                  !room.wagerRarity ||
                  !address ||
                  getCardCountByRarity(room.wagerRarity) > 0;
                ...
                return (
                  <motion.div
                    ...
                    className={`relative p-6 rounded-2xl border backdrop-blur-sm transition-all duration-300 ${
                      !userHasRequiredCards
                        ? "opacity-60 border-red-400/30 bg-red-500/10"
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        {/* Show wager rarity with user ownership status */}
                        {room.wagerRarity && (
                          <div className="flex items-center gap-2 mt-2">
                            <div
                              className={`px-2 py-1 rounded-full text-xs bg-gradient-to-r ${rarityInfo?.color} text-white font-medium`}
                            >
                              {rarityInfo?.name}
                            </div>
                            {address && (
                    ...
                    <motion.button
                      whileHover={{ scale: userHasRequiredCards ? 1.05 : 1 }}
                      whileTap={{ scale: userHasRequiredCards ? 0.95 : 1 }}
          ...
          >
            ...
            >
              <div className="text-center mb-6">
                ...
                {selectedRoom.wagerRarity && (
                  <div className="mt-2 text-sm text-cyan-300">
                    Requires: {getRarityInfo(selectedRoom.wagerRarity).name}{" "}
                    cards
                  </div>
                )}
              </div>
